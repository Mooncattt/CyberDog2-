...
class camera_picture_node(Node):
    def __init__(self,name):
      ...
      # self.sub_wifi_status = self.create_subscription(WifiStatus,mi_node + "wifi_status",self.wifi_status_callback,10)
        self.picture_date = self.create_subscription(Image,"/image",self.picture_date_callback,10)
        self.camera_state = self.create_client(CameraService,"camera_service")
...
def main(args=None):
    ...
    node = camera_picture_node("my_node_py")
